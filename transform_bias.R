...
save(df,file="df_transform_bias.Rdata")
...
out<-mclapply(pars,simfun,mc.cores=25)
df<-data.frame(do.call("rbind",out))
save(df,file="df_transform_power.Rdata")
...
pf<-function(df,var,...) {
    df$yv<-df[[var]]
    L<-split(df,df$N)
    plot(NULL,xlim=c(0,1),ylim=c(0,.6),xlab=expression(lambda),...)
...
legend("bottomright",bty='n',lty=c(2,2),lwd=2,col=rep(cols,1),c(
                                                                expression("LM estimate"~lambda~"=0.25"),
                                                                expression("LM estimate"~lambda~"=0.75")
                                                               )
       ,cex=.7)
