...
lambda<-c(.1,.5,.9) #cc<-seq(-3,-1,length.out=3)
alpha<-0
...
simfun<-function(pars,b1=1,b2=1,s2=0.25) {
...
pdf("/home/bd/Dropbox/Apps/Overleaf/Interaction_problems/transform_intuition.pdf",width=6,height=2.3)
...
for (i in 1:length(L)) {
    ...
    plot(z$x,z$y,
         pch=19,cex=.375,col=col,xlab='x',ylab='y')
    ...
    mtext(side=3,line=0,bquote(lambda~"="~.(lambda[i])))
}
dev.off()

pdf("/home/bd/Dropbox/Apps/Overleaf/Interaction_problems/transform_kidney.pdf",width=6,height=2.3)
par(mfrow=c(1,3),mgp=c(2,1,0),mar=c(3,3,1.8,0.2))
for (i in 1:length(L)) {
    z<-L[[i]]
    cc<-col2rgb("lightgray")
    c2<-rgb(cc[1],cc[2],cc[3],max=255,alpha=105)
    plot(z$x*z$z,z$y,pch=19,cex=.75,col=c2,ylim=c(0,4),xlab='x*z',ylab='y')
    z2<-z[z$y!=z$ystar,]
    cc<-col2rgb("red")
    ...
    legend("topleft",bty='n',#fill=c("red","lightgray"),
           c(paste("r(y,x*z)=",round(c1,2)),paste("r(ystar,x*z)=",round(c2,2))))
}
dev.off()
