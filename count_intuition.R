...
L<-list()
b2L<-seq(0.1,.45,length.out=3)
for (b2 in b2L) L[[as.character(b2)]]<-simfun(pars[[1]],b2=b2)


pdf("/home/bd/Dropbox/Apps/Overleaf/Interaction_problems/count_intuition.pdf",width=6,height=2.3)
...
for (i in 1:length(L)) {
    ...
    plot(z$x,
         z$ystar,pch=19,cex=.375,col=col,xlab='x',ylab='E(y)',
         ylim=c(0,7))
    m0<-glm(y~x*z,z,family='poisson')
    ...
    #legend("bottomright",bty='n',lty=c(2,1),lwd=3,c(paste("p(lm)=",round(summary(m)$coef[4,4],2)),paste("p(glm)=",round(summary(m0)$coef[4,4],2))),cex=.7)
}
dev.off()
